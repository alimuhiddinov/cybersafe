import { PrismaClient, ActivityType, CompletionStatus } from '@prisma/client';
...
async function testProgressTracking() {
  try {
    ...
    const startProgress = await progressService.startModule(testUser.id, testModule.id);
    ...
    const updateProgress = await progressService.updateProgress(testUser.id, testModule.id, updateData);
    ...
    const completeProgress = await progressService.completeModule(testUser.id, testModule.id, 600);
    ...
    const activityLog = await progressService.getUserActivityHistory(testUser.id, 1, 10);
    ...
    const achievementsResult = await progressService.getUserAchievements(testUser.id);
    
    console.log('✅ User achievements:', achievementsResult?.achievements?.length || 0);
    console.log('Achievement details:', JSON.stringify(achievementsResult?.achievements || [], null, 2));
    
    console.log('\n--- Test 3.2: Achievement Association ---');
    console.log(`✅ Achievements associated with user ${testUser.id}: ${(achievementsResult?.achievements || []).every(a => a.awardedAt)}`);
    ...
    const savedNotes = await progressService.saveNotes(testUser.id, testModule.id, testNotes);
    ...
    const savedBookmarks = await progressService.saveBookmarks(testUser.id, testModule.id, JSON.stringify(testBookmarks));
