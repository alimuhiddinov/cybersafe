...
async function testProgressFeatures() {
  try {
    ...
    const userAchievements = await progressService.getUserAchievements(testUser.id);
    console.log(`✅ User has ${userAchievements?.achievements?.length || 0} achievements`);
    ...
    for (let i = 1; i < 3; i++) { // Complete 2 more modules
      await progressService.startModule(testUser.id, testModules[i].id);
      await progressService.completeModule(testUser.id, testModules[i].id, 500);
      console.log(`✅ Completed module ${i+1}`);
    }
    
    await progressService.checkAndAwardAchievements(testUser.id);
    const updatedAchievements = await progressService.getUserAchievements(testUser.id);
    console.log(`✅ User now has ${updatedAchievements?.achievements?.length || 0} achievements`);
