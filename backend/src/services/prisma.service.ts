import { PrismaClient, UserRole, CompletionStatus } from '@prisma/client';
...
class PrismaService {
   ...
   */
  async getAssessmentWithQuestions(assessmentId: number) {
    try {
   ...
   */
  async getUserAssessmentAttempts(userId: number) {
    try {
  ...
  async getLeaderboard(limit: number = 10) {
    try {
      ...
      const leaderboard = await Promise.all(
        userProgressSums.map(async (progress) => {
  ...
  async getUserDashboard(userId: number) {
    try {
      ...
      const moduleStats = {
        completed: progress.filter((p) => p.completionStatus === CompletionStatus.COMPLETED).length,
        inProgress: progress.filter((p) => p.completionStatus === CompletionStatus.IN_PROGRESS).length,
        notStarted: progress.filter((p) => p.completionStatus === CompletionStatus.NOT_STARTED).length,
