...
class FeedbackService {
   ...
   */
  async getModuleFeedback(moduleId: number, page = 1, pageSize = 10) {
   ...
   */
  async getUserFeedback(userId: number, page = 1, pageSize = 10) {
    const skip = (page - 1) * pageSize;
    const take = pageSize;
   ...
   */
  async deleteFeedback(feedbackId: number, requestingUserId: number, isAdmin: boolean) {
    try {
   ...
   */
  async getModuleFeedbackAnalytics(moduleId?: number) {
    try {
      ...
      return {
        ...
        recentTrends: {
          ...
          ratingChange: calculatePercentageChange(
            ...
            recentRatings._avg.rating || 0
          ),
          volumeChange: calculatePercentageChange(
            previousRatings._count.rating || 0,
            recentRatings._count.rating || 0
          )
        }
      };
    } catch (error) {
