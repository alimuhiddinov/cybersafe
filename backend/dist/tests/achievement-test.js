...
async function testAchievementTracking() {
    try {
        ...
        const initialAchievements = await progress_service_1.default.getUserAchievements(testUser.id);
        ...
        await progress_service_1.default.startModule(testUser.id, testModules[0].id);
        await progress_service_1.default.completeModule(testUser.id, testModules[0].id, 300);
        console.log('- Module completed');
        // Check for achievement award
        await progress_service_1.default.checkAndAwardAchievements(testUser.id);
        const firstModuleResult = await progress_service_1.default.getUserAchievements(testUser.id);
        console.log(`- User now has ${firstModuleResult.achievements.length} achievements`);
        const hasFirstModuleAchievement = firstModuleResult.achievements.some(a => a.name === 'FIRST_MODULE_COMPLETED');
